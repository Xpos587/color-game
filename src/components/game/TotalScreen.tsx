...
import { getTotalFeedback, hsbToCss } from "@/lib/color"
...
export function TotalScreen({ totalScore, roundScores, onPlayAgain, sound }: TotalScreenProps) {
  ...
  return (
    <div className={`total-screen${revealed ? "revealed" : ""}`}>
      ...
      <div
        id="total-actions"
        style={{ margin: "19px 30px 0", display: "flex", flexDirection: "column" }}
      >
        <button onClick={onPlayAgain} onMouseEnter={() => sound.hover()} className="go-btn go-text">
          Play again
        </button>
      </div>
    </div>
