...
export function click(): void {
  ...
  setTimeout(() => tone(960, 0.08, 0.1, "sine"), 30)
}
...
export function soloHover(): void {
  tone(720, 0.09, 0.09, "sine")
}

export function soloClick(): void {
  tone(660, 0.12, 0.16, "triangle")
}
...
export function multiClick(): void {
  ...
  setTimeout(() => tone(660, 0.09, 0.1, "sine"), 25)
...
export function keystroke(): void {
  try {
    ...
    cd[1] = -0.8
    cd[2] = 0.5
    for (let i = 3; i < clickLen; i++)
      cd[i] = (Math.random() * 2 - 1) * Math.max(0, 1 - (i / clickLen) * 4)
...
export function hardOff(): void {
  try {
    ...
    ;[380, 520, 780].forEach((freq, i) => {
      ...
      g.gain.setValueAtTime(i === 2 ? 0.14 : 0.1, t + delay)
    ...
    sparkle.connect(gs)
    gs.connect(c.destination)
    sparkle.start(t + 0.14)
    sparkle.stop(t + 0.22)
  } catch {}
}
...
export function toDark(): void {
  ...
  setTimeout(() => tone(220, 0.2, 0.06, "sine"), 160)
}

export function toLight(): void {
  tone(330, 0.12, 0.08, "sine")
  setTimeout(() => tone(523, 0.12, 0.1, "sine"), 80)
  setTimeout(() => tone(660, 0.15, 0.1, "sine"), 160)
}
...
export function blipReady(): void {
  tone(600, 0.08, 0.1, "sine")
}
...
export function sliderReset(): void {
  _lastSliderTick = -999
}
...
function _flutterTeardown(): void {
  if (_flutterNoise) {
    ...
    try {
      _flutterNoise.disconnect()
    } catch {}
    try {
      _flutterBp?.disconnect()
    } catch {}
    try {
      _flutterGain?.disconnect()
    } catch {}
  }
  _flutterNoise = null
  _flutterBp = null
...
export function scoreTick(progress: number, isInt: boolean): void {
  try {
    const c = getCtx()
    if (!c) return
    const baseFreq = 523 * 2 ** (progress * 2)
...
export function scoreLand(): void {
  try {
    ...
    ;[1046, 1318, 1568].forEach((f, i) => {
      ...
      osc.connect(gain)
      gain.connect(c.destination)
      osc.start(c.currentTime + i * 0.03)
      osc.stop(c.currentTime + i * 0.03 + 0.2)
    })
  } catch {}
}
...
export function goLock(): void {
  try {
    ...
    sd[0] = 1
    sd[1] = -1
    for (let i = 2; i < snapLen; i++)
      sd[i] = (Math.random() * 2 - 1) * Math.max(0, 1 - (i / snapLen) * 3)
    const snapSrc = c.createBufferSource()
    snapSrc.buffer = snapBuf
    const res = c.createBiquadFilter()
    res.type = "bandpass"
    res.frequency.value = 2800
    res.Q.value = 5
...
export function startPlaying(): void {
  try {
    ...
    noiseSrc.start(chordT)
    noiseSrc.stop(chordT + 0.04)
  } catch {}
}
...
export function dailyDroneStart(): void {
  ...
  try {
    ...
    o1.connect(_dailyFilter)
    o1.start()
    const o2 = c.createOscillator()
    o2.type = "sine"
    o2.frequency.setValueAtTime(42, t)
    o2.connect(_dailyFilter)
    o2.start()
    const o3 = c.createOscillator()
    o3.type = "square"
    o3.frequency.setValueAtTime(80, t)
    const o3g = c.createGain()
    o3g.gain.setValueAtTime(0.15, t)
    o3.connect(o3g)
    o3g.connect(_dailyFilter)
    o3.start()
    const o4 = c.createOscillator()
    o4.type = "sine"
    o4.frequency.setValueAtTime(20, t)
    const o4g = c.createGain()
    o4g.gain.setValueAtTime(0.5, t)
    o4.connect(o4g)
    o4g.connect(_dailyFilter)
    o4.start()
    ...
    lfo.connect(lfoGain)
    lfoGain.connect(_dailyFilter.frequency)
    lfo.start()
...
export function challengeSend(): void {
  try {
    ...
    chargeG.gain.linearRampToValueAtTime(0.18, t + 0.1)
    chargeG.gain.exponentialRampToValueAtTime(0.001, t + 0.14)
    chargeSrc.connect(chargeBp)
    chargeBp.connect(chargeG)
    chargeG.connect(c.destination)
    chargeSrc.start(t)
    chargeSrc.stop(t + 0.14)
    ...
    const kg = c.createGain()
    kg.gain.setValueAtTime(0.2, t + burst)
    kg.gain.exponentialRampToValueAtTime(0.001, t + burst + 0.1)
    kick.connect(kg)
    kg.connect(c.destination)
    kick.start(t + burst)
    kick.stop(t + burst + 0.1)
...
function _initRobotVoice(): void {
  ...
  _robotVoice =
    ...
    voices.find(
      v => /en.*us|en.*gb|english/i.test(v.lang) && /compact|robot|synth/i.test(v.name),
    ) ||
    null
  _robotReady = true
}
...
function robotSay(
  text: string,
  opts?: { delay?: number; pitch?: number; rate?: number; volume?: number },
): void {
...
export function robotCopied(): void {
  robotSay("link copied")
}
export function robotSolo(): void {
  robotSay("single player", { delay: 150, pitch: 0.1, rate: 0.85, volume: 0.8 })
}
export function robotMulti(): void {
  robotSay("multiplayer", { delay: 150, pitch: 0.1, rate: 0.8, volume: 0.85 })
}
export function robotEasy(): void {
  robotSay("easy", { delay: 200, pitch: 0.8, rate: 0.7, volume: 0.5 })
}
export function robotHardcore(): void {
  robotSay("hard", { delay: 200, pitch: 0.01, rate: 0.55, volume: 0.9 })
}
export function robotHighscores(): void {
  robotSay("high scores", { delay: 100 })
}
export function robotDaily(): void {
  robotSay("daily mode", { delay: 150, pitch: 0.01, rate: 0.5, volume: 0.95 })
}
